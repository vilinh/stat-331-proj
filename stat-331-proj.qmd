...
To visualize the data, we made two graphs to visualize the relationship between GDP and Child Mortality. The first graph plots the average GDP per capita per year and the average child mortality rate per year over time. We have two different scales for the y axis as the numbers for the mortality rate and GDP are on different scales, and this enables to us to see the changes in both on the same graph. At a glance, the average GDP per Capita has increased over time while the child mortality rate has decreased over time. It seems that there an increase in GDP is correlated with a decrease in mortality rate.

The second graph plots the average GDP per capita (over the years per country) against the average mortality rate (over the years per country). At a glance, it again looks like a higher mortality rate is correlated with a lower GDP per capita as the plotted points start at the top and curve downwards.
...
# | code-fold: true
joined_df |>
  mutate(Decade = cut(Year, breaks = seq(1950, 2020, by = 10))) |>
  ggplot(aes(x = `GDP Per Capita`, y = `Mortality Rate`)) +
  geom_point(alpha = 0.25) +
  facet_wrap(~ Decade, scales = "free") +
  labs(title = "Scatter Plot of GDP Per Capita vs Mortality Rate Over Time",
       x = "GDP Per Capita",
       y = "Mortality Rate") +
  theme_minimal()
```
...
### Linear Regression
Here, we create a linear model where the response variable is the child mortality rate and the explanatory variable is the GDP per capita. We want to see how child mortality is affected by changes in GDP, according to linear regression. We get an estimated intercept of 128.411569954 and a slope of -0.002538529. So according to the model, when the GDP is 0, the mortality rate is around 128, and for each 1 unit increase in GDP, the mortality rate decreases by .0025. 
```{r}
# | code-fold: true
