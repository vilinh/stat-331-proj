...
>>>>>>> caa881b706b842869afeb54db57aaab53252c622
...
### Model Fit
To assess the regression model, we look at the variability in the response values that were accounted for by it. We look at the variance in the response values, the fitted values, and the residuals to determine the quality of our model.
```{r}
# | code-fold: true
...
```
From the variance of the response values and the residuals, we can calculate R-squared using the formula $R^2 = 1 - \frac{{\text{var}( \hat{y})}}{{\text{var}(y)}}$, with $\hat{y}$ being the variance of the residuals and $y$ being the variance of the response values. Plugging these values into our formula, we get an $R^2$ value of approximately 0.3258. This means that our regression model explains approximately 32.58% of the variability of our response values. This indicates that our model may not be of the best quality because there is still a large portion of the variability in the response variable that is unexplained. 
...
```{r}
mult_pred <- function(sd) {
  set.seed(42)
  data <- data.frame(
    "GDP Per Capita" = rnorm(13845, mean = 0, sd = sd),
   "Mortality Rate" = rnorm(13845, mean = 0, sd = sd)
  )
  return(data)
}
...
```{r}
perform_regression <- function(observed_data, simulated_datasets) {
  data <- na.omit(data.frame(observed_data, simulated_datasets))
  lm_model <- lm(`Mortality Rate` ~ `GDP Per Capita`, data = data)
...
```{r}
observed_data <- joined_df %>%
  select("Mortality Rate", "GDP Per Capita")
simulated_datasets <- map(1:1000, ~ mult_pred(sd = std_error))
combined_simulated_data <- bind_rows(simulated_datasets)

rsquared_values <- map_dbl(simulated_datasets, ~ perform_regression(observed_data, .x))
```
```{r}
str(observed_data)
str(combined_simulated_data)
colnames(joined_df)
```
