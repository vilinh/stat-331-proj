...
set.seed(42)
data <- data.frame(
    gdp = rnorm(13845, mean = 0, sd = sd),
    mortality = rnorm(13845, mean = 0, sd = sd)
  )
  return(data)
}
...
perform_regression <- function(observed_data, simulated_datasets) {
  ...
  lm_model <- lm(observed_data ~ simulated_datasets, data = data)
...
```{r}
observed_data <- joined_df |> select("Mortality Rate", "GDP Per Capita")
simulated_datasets <- map(1:1000, ~ mult_pred(sd = std_error))
rsquared_list <- map_dbl(simulated_datasets, ~ perform_regression(observed_data, .x))
