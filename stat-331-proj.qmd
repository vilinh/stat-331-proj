...
variance_table
```
Because we know the variance of the response values and the residuals, we can calculate R-squared using the formula R-squared = var(y hat) / var(y), with var(y hat) being the variance of the fitted values and var(y) being the variance of the response values. Plugging these values into our formula, we get an R-squared of approximately 0.3258. This means that our regression model explains approximately 32.58% of the variability of our response values. This indicates that our model may not be of the best quality because there is still a large portion of the variability in the response variable that is unexplained. 
