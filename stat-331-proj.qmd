...
observed_plot <- ggplot(year_avgs, aes(x = Avg_GDP_Per_Capita, y = Avg_Mortality_Rate)) +
  ...
  theme_minimal() +
  labs(title = "Observed Average Mortality Rate vs. Average GDP Per Capita",subtitle = "Average Mortality Rate", x= "Average GDP Per Capita", y = "",
       color = "Data Type")
...
simulated_plot <- ggplot(simulated_year_avgs, aes(x = Avg_GDP_Per_Capita, y = Simulated_Mortality)) +
  ...
  theme_minimal() +
  labs(title = "Simulated Average Mortality Rate vs. Average GDP Per Capita", subtitle = "Simulated Mortality Rate", x= "Average GDP Per Capita", y = "",
       color = "Data Type")
...
# Assuming mortalitygdp_lm and year_avgs are already defined and correct
...
# 3. Prepare for visualization with proper labeling
simulated_year_avgs <- year_avgs %>%
    mutate(Simulated_Mortality = predictions + simulated_errors) %>%
    rename(Average_GDP_Per_Capita = Avg_GDP_Per_Capita,
           Average_Mortality_Rate = Avg_Mortality_Rate)

# Plot for observed data with confidence bands
observed_plot <- ggplot(simulated_year_avgs, aes(x = Average_GDP_Per_Capita, y = Average_Mortality_Rate)) +
  geom_point(aes(color = "Observed"), alpha = 0.6) +
...
# Plot for simulated data with confidence bands
simulated_plot <- ggplot(simulated_year_avgs, aes(x = Average_GDP_Per_Capita, y = Simulated_Mortality)) +
  geom_point(aes(color = "Simulated"), alpha = 0.6) +
  ...
  labs(title = "Simulated Average Mortality Rate vs Average GDP Per Capita",
       color = "Data Type",
       x = "Average GDP Per Capita",
       y = "Simulated Mortality Rate")

# Combine plots side by side with shared legend
combined_plot <- (observed_plot | simulated_plot) + 
