...
then. The "Mortality Rate" variable represents the probability of dying between birth and age 5 per 1000 live births. Finally, the "GDP Per Capita" variable represents the 
...
### Model Fit
Based on the results of our linear model, we find that the child mortality rate when GDP is zero is 128.4. We also find that, holding other factors constant, a one-unit ($1000 USD) increase in GDP corresponds to the child mortality rate decreasing -0.0025 on average. 
...
```
From the variance of the response values and the residuals, we can calculate R-squared using the formula $R^2 = \frac{{\text{var}( \hat{y})}}{{\text{var}(y)}}$, with $\hat{y}$ being the variance of the fitted values and $y$ being the variance of the response values. Plugging these values into our formula, we get an $R^2$ value of approximately 0.3258. This means that our regression model explains approximately 32.58% of the variability of our response values. This indicates that our model may not be of the best quality because there is still a large portion of the variability in the response variable that is unexplained. 
